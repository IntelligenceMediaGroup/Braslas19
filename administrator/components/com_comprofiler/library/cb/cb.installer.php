...
* @version $Id: cb.installer.php 1617 2011-11-01 10:34:50Z beat $
...
class cbInstaller {
	...
	*/
	function parseFiles( $tagName='files', $special='', $specialError='', $adminFiles=0 ) {
		...
		$result					=	$this->copyFiles( $installFrom, $installTo, $copyfiles, $this->installMethod() );
	...
	*/
	...
	function doCleanup( $p_docleanup = null ) {
		return $this->setVar( 'i_docleanup', $p_docleanup );
	}

	function installMethod( $p_method = null ) {
		return ( $this->setVar( 'i_installmethod', $p_method ) != 'install' );
	}
	...
	*/
	...
	function cleanupInstall( $userfile_name, $resultdir) {
		if ( $this->doCleanup() && file_exists( $resultdir ) ) {
			$adminFS		=&	cbAdminFileSystem::getInstance();
