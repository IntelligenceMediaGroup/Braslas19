...
* @version $Id: comprofiler.html.php 1580 2011-10-28 16:36:37Z beat $
	...
	for (var i=0; i < me.length; i++) {
		...
		if ( (typeof(me[i].getAttribute('mosReq')) != "undefined") && ( me[i].getAttribute('mosReq') == 1) ) {
			...
			if (me[i].value == '') {
				...
				errorMSG += me[i].getAttribute('mosLabel') + " <?php echo CBTxt::html_entity_decode(_UE_LENGTH_ERROR); ?> " + (me[i].value.length - me[i].getAttribute('mosLength')) + " <?php echo CBTxt::html_entity_decode(_UE_CHARACTERS); ?>\n";
				// notify user by changing background color, in this case to red
				if (cbDefaultFieldBackground === undefined) cbDefaultFieldBackground = ((me[i].style.getPropertyValue) ? me[i].style.getPropertyValue("backgroundColor") : me[i].style.backgroundColor);
				me[i].style.backgroundColor = "red";
				iserror=1;
...
?>
	...
	echo sprintf(_UE_EMAILFORMTITLE,"<a href=\"".cbSef( "index.php?option=com_comprofiler&amp;task=userProfile&amp;user=".$rowTo->id )."\">".getNameFormat($rowTo->name,$rowTo->username,$ueConfig['name_format'])."</a>");
	?></div>
...
<?php echo $tabs->fieldJS; ?>
	if (me['username'].value == "") {
		errorMSG += "<?php echo CBTxt::html_entity_decode(_REGWARN_UNAME);?>\n";
		me['username'].style.backgroundColor = "red";
		iserror=1;
	} else if (r.exec(me['username'].value) || (me['username'].value.length < 3)) {
		errorMSG += "<?php printf( CBTxt::html_entity_decode(_VALID_AZ09), CBTxt::html_entity_decode(_PROMPT_UNAME), 2 );?>\n";
		me['username'].style.backgroundColor = "red";
		iserror=1;
	...
	if ((me['password'].value) && (me['password'].value.length < 6)) {
		errorMSG += "<?php printf( CBTxt::html_entity_decode(_VALID_AZ09), CBTxt::html_entity_decode(_REGISTER_PASS), 6 );?>\n";
...
$('#cbbtncancel').click( function() {
	window.location='<?php echo cbSef( 'index.php?option=' . htmlspecialchars( cbGetParam( $_REQUEST, 'option' ) ) . ( ( $user->id == $_CB_framework->myId() ) ? '' : ( '&amp;user=' . $user->id ) ) . getCBprofileItemid( true ), false ); ?>';
	return false;
} );
<?php
		$cbjavascript	=	ob_get_contents();
...
<div class="cbEditProfile"><div id="cbEditProfileInner" class="cbHtmlEdit">
	...
	echo HTML_comprofiler::_cbTemplateRender( $cbTemplate, $user, 'Profile', 'drawEditProfile', array( &$user, $tabcontent, $submitvalue, _UE_CANCEL, $bottomIcons ), $output );
?>
...
<?php	if($ueConfig['reg_enable_toc']) { ?>
	if(!me['acceptedterms'].checked) {
		errorMSG += "<?php echo CBTxt::html_entity_decode(_UE_TOC_REQUIRED); ?>\n";
		iserror=1;
	}
<?php	} ?>
	// loop through all input elements in form
